...
typedef enum
{
  CLI_STATE_NORMAL, // 일반 텍스트 입력 상태
  CLI_STATE_ESC,    // ESC(0x1B) 수신 직후
  CLI_STATE_CSI     // CSI(Control Sequence Introducer, '[') 수신 후
} CliState_t;
...
{
{ "help", cmd_help, "Show all commands.", "Usage: "_ANSI(FG_CYAN, BG_BLACK)"help"ANSI_RESET" "_ANSI(FG_YELLOW, BG_BLACK)"[command]"ANSI_RESET"\r\n"
...
static void cmd_clear(int argc, char** argv)
{
  uart_printf(ANSI_RESET"\x1B[0;0f\x1B[J");
}
...
void cli_run(void)
{
  ...
  { 0 };
  static CliState_t state = CLI_STATE_NORMAL; // Escape Sequence 처리를 위한 상태 변수

  char c = uart_ReadChar();

  // 수신된 데이터가 없으면 리턴 (Non-blocking)
  //if( c <= 0 ) return;
//  reprocess:
...
//              uart_printf("in %s, buffer: %s\r\n", __func__, buffer); // dbg
          ...
          case '\b':   // Backspace (Standard)
            if( idx > 0 )
            {
              idx--;
              buffer[idx] = '\0';
              uart_printf("\b \b"); // 커서 뒤로 -> 공백 출력(지우기) -> 커서 다시 뒤로
            }
            break;
      ...
      break;
    } // END case CLI_STATE_NORMAL

    case CLI_STATE_ESC:
    {
      // ESC 다음에는 보통 '[' 문자가 옴 (CSI: Control Sequence Introducer)
      if( c == '[' )
      {
        state = CLI_STATE_CSI;
      }
      ...
      {
        // '['가 아니면 Escape 시퀀스가 아님. 다시 일반 모드로 복귀
        uart_printf("\x1B[K");
        memset(buffer, 0, sizeof(buffer));
        idx = 0;
        buffer[idx++] = c;
...
//        goto reprocess;
    ...
    case CLI_STATE_CSI:
    {
      ...
      {
        case 'A': // Up Arrow (위)
          // [TODO] 히스토리 기능 구현 시 여기에 "이전 명령어 불러오기" 로직 추가
          // cli_history_prev(buffer, &idx);
          break;

        case 'B': // Down Arrow (아래)
          // [TODO] 히스토리 기능 구현 시 여기에 "다음 명령어 불러오기" 로직 추가
          break;
